...
const EditProposalHeaderInputCompany = React.forwardRef((props, ref) => {
  const {
    form,
    companySource,
    setCompanySource,
  ...
  const [companyNoResultsMessage, setCompanyNoResultsMessage] = useState(null)
    ...
    getCompanies(name, id, isExact)
      .then(records => {
        if (internalProcessCompanySearchId === processCompanySearchId) {
          const source = []
          setCompanyNoResultsMessage(null)
          if (records.length > 0) {
            records.map(
              record =>
                source.push({
                  ...
                  requiredZipCode: record?.obrigarOrganizacaoCEP,
                  personAddress: record?.personAddresses,
                }),
            )
          } else {
...
EditProposalHeaderInputCompany.propTypes = {
  form: PropTypes.any,
...
export default EditProposalHeaderInputCompany
