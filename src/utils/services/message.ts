 ...
 *
 */
export function prepareBody(personData) {
   ...
   *   */   
    const filterContacts = personData?.personContacts
      .map(r => ({
        ...
        phone: r?.phone,
        cellphoneId:
          r?.tipoContatoCellPhone === 'Cel' ||
          r?.tipoContatoCellPhone === 'Cel2'
            ? r?.cellPhoneId
            : 0,
    ...
    const dividedFilteredContacts: IFilteredPersonContactBody = filterContacts.map(r => {
    const contactTypeIds = [{id:r?.emailId, guid:r?.personContactEmailGuid, type:"Email"}, {id:r?.phoneId, guid:r?.personContactPhoneGuid, type:"Phone"}, {id:r?.cellphoneId, guid:r?.personContactCellPhoneGuid, type:"Cellphone"}, {id:r?.siteId, guid:r?.personContactSiteGuid, type:"Site"}]  
  /** Hotfix, precisava terminar rápido. Então gambiarra pra ajeitar o json */    
    return contactTypeIds.map((currentValue) => currentValue?.id !== undefined && currentValue?.id !== 0 && currentValue?.guid !== "00000000-0000-0000-0000-000000000000" && {
      ...
      cellphoneId: currentValue.type === "Cellphone" ? r?.cellphoneId : 0,
      cellphone: currentValue.type === "Cellphone" ? r?.cellphone : null,
      ...
      return dividedFilteredContacts
  }
